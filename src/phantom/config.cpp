...
namespace alpr
{
  ...
  Config::Config(const std::string country, const std::string config_file, const std::string runtime_dir)
  {
    ...
    {
      std::cerr << "{\"error\": \"The configuration file '" << config_file_path << "' does not exist.\"}" << endl;
      return;
    }
    else if (DirectoryExists(config_file_path.c_str()))
    {
      std::cerr << "{\"error\": \"The configuration file '" << config_file_path << "' is a directory, not a file.\"}" << endl;
      return;
    }
    ...
    if (DirectoryExists(this->runtimeBaseDir.c_str()) == false)
    {
      std::cerr << "{\"error\": \"The run-time directory '" << this->runtimeBaseDir << "' does not exist.\"}" << endl;
      return;
    }
  ...
  bool Config::load_countries(const std::string countries) {
    ...
    {
      std::cerr << "{\"error\": \"Country not specified.\"}" << endl;
      return false;
    }
  ...
  bool Config::setCountry(std::string country)
  {
    ...
    {
      std::cerr << "{\"error\": \"The country configuration file '" << country_config_file << "' does not exist. Missing configuration for the country '" << country<< "'.\"}" << endl;
      return false;
    }
    ...
    {
      std::cerr << "{\"error\": \"The run-time directory '" << this->runtimeBaseDir << "' is invalid.  Missing OCR data for the country '" << country<< "'.\"}" << endl;
      return false;
    }
